...
class AppointmentController extends Controller
{
    ...
    public function followUpForm(int $id)
    {
        ...
        // allow follow-up ONLY if completed (and ideally past start time) and not canceled
        if ($appointment->status !== 'completed') {
            return redirect()
                ...
                ->with('swal', [
                    'icon'  => 'warning',
                    'title' => 'Not allowed',
                    'text'  => 'You can create a follow-up only for completed appointments.',
                ]);
        }

        // (optional) require counselor
        if (empty($appointment->counselor_id)) {
            return redirect()
                ...
                ->with('swal', [
                    'icon'  => 'warning',
                    'title' => 'Not allowed',
                    'text'  => 'Assign a counselor first.',
                ]);
        }

        return view('admin.appointments.follow-up', compact('appointment'));
    }

    public function followUpStore(Request $r, int $id): \Illuminate\Http\RedirectResponse
    {
        $parent = \DB::table('tbl_appointments')->where('id', $id)->first();
        abort_unless($parent, 404);

        if ($parent->status !== 'completed') {
            return redirect()->route('admin.appointments.show', $id)->with('swal', [
                'icon'  => 'warning',
                'title' => 'Not allowed',
                'text'  => 'You can create a follow-up only for completed appointments.',
            ]);
        ...
        $scheduledAt = \Carbon\Carbon::parse($data['date'].' '.$data['time'].':00');

        \DB::table('tbl_appointments')->insert([
            'student_id'   => $parent->student_id,
            'counselor_id' => $parent->counselor_id, // keep same counselor
            'scheduled_at' => $scheduledAt,
            'status'       => 'pending',
            'created_at'   => now(),
            'updated_at'   => now(),
            // optional linking if you added a column:
            // 'parent_id'    => $parent->id,
            'final_note'   => $data['note'] ?? null,
        ]);

        return redirect()->route('admin.appointments.index')->with('swal', [
            'icon'  => 'success',
            'title' => 'Follow-up created',
            'text'  => 'A new follow-up appointment has been added.',
        ]);
    }
}