...
<div class="max-w-5xl mx-auto p-4 sm:p-6 space-y-6">
  ...
  <section class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm">
    <div class="px-4 sm:px-6 pt-4 pb-3">
      <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Display &amp; Accessibility</h3>
      <p class="text-sm text-gray-500 dark:text-gray-400">Theme, text size, and motion comfort.</p>
    </div>
      ...
      <div class="compact-row flex items-center justify-between gap-4 px-4 sm:px-6 py-3.5">
        ...
        <label for="darkModeToggle" class="relative inline-flex items-center cursor-pointer select-none">
          ...
          <span class="block w-14 h-8 rounded-full border transition-all duration-300
                       bg-black dark:bg-gray-800 border-gray-300 dark:border-gray-600
                       peer-focus-visible:outline-none peer-focus-visible:ring-2 peer-focus-visible:ring-indigo-500
                       peer-checked:bg-transparent peer-checked:border-indigo-400
                       peer-checked:shadow-[0_0_14px_rgba(79,70,229,0.55)]
                       dark:peer-checked:shadow-[0_0_26px_rgba(79,70,229,0.85)]"></span>
          <span class="absolute left-1 top-1 w-6 h-6 rounded-full bg-white shadow
                       transition-transform duration-300
                       peer-checked:translate-x-6
                       peer-checked:shadow-[0_0_16px_rgba(79,70,229,0.8)]"></span>
        </label>
      </div>
  ...
  <section class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm">
    <div class="px-4 sm:px-6 pt-4 pb-3">
      <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Chat Cleanup</h3>
      <p class="text-sm text-gray-500 dark:text-gray-400">Auto-delete old conversations.</p>
    </div>
  ...
  <section class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm">
    <div class="px-4 sm:px-6 pt-4 pb-3">
      ...
      <p class="text-sm text-gray-500 dark:text-gray-400">Contact the team or report an issue.</p>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function autoSave(payload){
    ...
    fetch(saveUrl, { method: 'POST', headers: { 'X-CSRF-TOKEN': csrf }, body: fd }).then(()=> toast('Saved'));
  }
  ...
  // Auto-delete
  function clampDays(val){ if (val === '' || val === null) return ''; const n = Number(val); if (!Number.isFinite(n)) return ''; return Math.min(365, Math.max(0, n)); }
  ...
  autoDeleteInput?.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); autoDeleteInput.blur(); }});
